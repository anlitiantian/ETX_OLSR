...
private:
  ...
  //判断节点是否在小区域内
  bool isInMarginArea(const Vector &position);
  ...
  double m_Wmax;							//最大角速度
  //私有，不可设置
  double m_distance_margin;				//边界阈值，设为2*Vmean/Wmax
  Vector2D m_centerPosition;			//超过边界阈值后做转弯的圆心坐标
  bool m_isCircle;						//判断上次的运动是否是圆周运动
