...
namespace ns3
{
  ...
  double GaussMarkovMobilityModel::GetVelocityNum()
  {
    return m_Velocity;
  }
  double GaussMarkovMobilityModel::GetDirection()
  {
    return m_Direction;
  }
  double GaussMarkovMobilityModel::GetPitch()
  {
    return m_Pitch;
  }
  ...
  GaussMarkovMobilityModel::DoWalk(Time delayLeft)
  {
    ...
    if (isInMarginArea(nextPosition))
    {                         //在小区域内，不用改变运动状态
  ...
  {
    return position.x <= m_bounds.xMax - m_distance_margin && position.x >= m_bounds.xMin + m_distance_margin &&
