...
#define OLSR_C 0.0625
        ...
        MessageHeader::GetSerializedSize(void) const
        {
            ...
            case HNA_MESSAGE:
                ...
                break;
            case HELLOACK_MESSAGE:
                size += m_message.helloAck.GetSerializedSize();
                break;
            default:
                NS_ASSERT(false);
            }
        ...
        MessageHeader::Serialize(Buffer::Iterator start) const
        {
            ...
            case HNA_MESSAGE:
                ...
                break;
            case HELLOACK_MESSAGE:
                m_message.helloAck.Serialize(i);
                break;
            default:
                NS_ASSERT(false);
            }
        ...
        MessageHeader::Deserialize(Buffer::Iterator start)
        {
            ...
            NS_ASSERT(m_messageType >= HELLO_MESSAGE && m_messageType <= HELLOACK_MESSAGE);
            m_vTime = i.ReadU8();
            ...
            case HNA_MESSAGE:
                ...
                break;
            case HELLOACK_MESSAGE:
                size += m_message.helloAck.Deserialize(i, m_messageSize - OLSR_MSG_HEADER_SIZE);
                break;
            default:
                NS_ASSERT(false);
            }
        ...
        MessageHeader::Hello::Serialize(Buffer::Iterator start) const
        {
            ...
            i.WriteU16(this->vel_x);
            i.WriteU16(this->vel_y);
            i.WriteU16(this->vel_z);
                 ...
                 iter != this->linkMessages.end(); iter++)
            {
                ...
                //这里是因为修改了hello包的底层结构，因此linkMessage的长度扩展为原本的两倍
                i.WriteHtonU16(4 + lm.neighborInterfaceAddresses.size() * IPV4_ADDRESS_SIZE * 2);
                ...
                for (std::vector<uint32_t>::const_iterator etx_iter = lm.neighborEtxs.begin();
                     etx_iter != lm.neighborEtxs.end(); etx_iter++)
                {
                    i.WriteHtonU32(*etx_iter); // 这里需要写入以上邻居地址的ETX倒数[方便hello包的序列化]
                }
            }
        }
        ...
        MessageHeader::Hello::Deserialize(Buffer::Iterator start, uint32_t messageSize)
        {
            ...
            this->vel_x = i.ReadU16();
            this->vel_y = i.ReadU16();
            this->vel_z = i.ReadU16();
            ...
            while (helloSizeLeft)
            {
                ...
                for (int n = (lmSize - 4) / (IPV4_ADDRESS_SIZE * 2); n; --n)
                {
                    lm.neighborInterfaceAddresses.push_back(Ipv4Address(i.ReadNtohU32()));
                }
                for (int n = (lmSize - 4) / (IPV4_ADDRESS_SIZE * 2); n; --n)
                {
                    lm.neighborEtxs.push_back(i.ReadNtohU32()); // 将邻居etx存入记录表
                }
        ...
        {
            return 4 + this->neighborAddresses.size() * IPV4_ADDRESS_SIZE * 2; //由于新增了ETX数据，需要填充一倍大小
        }
        ...
        MessageHeader::Tc::Serialize(Buffer::Iterator start) const
        {
            ...
            for (std::vector<uint32_t>::const_iterator iter = this->EtxData.begin();
                 iter != this->EtxData.end(); iter++)
            {
                i.WriteHtonU32(*iter); // 写入邻居的etx
            }
        }
        ...
        MessageHeader::Tc::Deserialize(Buffer::Iterator start, uint32_t messageSize)
        {
            ...
            int numAddresses = (messageSize - 4) / (2 * IPV4_ADDRESS_SIZE);
            this->neighborAddresses.clear();
            for (int n = 0; n < numAddresses; ++n)
            {
            ...
            for (int n = 0; n < numAddresses; ++n)
            {
                this->EtxData.push_back(i.ReadNtohU32());
            }
            return messageSize;
        }
        ...
        uint32_t
        MessageHeader::HelloAck::GetSerializedSize(void) const
        {
            return 1 * IPV4_ADDRESS_SIZE; //只有一个ip地址
        }

        void
        MessageHeader::HelloAck::Print(std::ostream &os) const
        {
            /// \todo
        }

        void
        MessageHeader::HelloAck::Serialize(Buffer::Iterator start) const
        {
            Buffer::Iterator i = start;
            // 填充接收方地址
            i.WriteHtonU32(this->receiveAddress.Get());
        }

        uint32_t
        MessageHeader::HelloAck::Deserialize(Buffer::Iterator start, uint32_t messageSize)
        {
            Buffer::Iterator i = start;

            NS_ASSERT(messageSize >= 4);

            this->receiveAddress = Ipv4Address(i.ReadNtohU32());
