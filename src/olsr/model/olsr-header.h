...
public:
  ...
  */
  ...
  struct Tc
  {
    ...
    std::vector<uint32_t> EtxData; //!< origin Etx DATA 这里指代etx的倒数，
    /**
     * This method is used to print the content of a MID message.
     * \param os output stream
     */
  ...
  */
  ...
  {
    // 记录邻居主地址
    Ipv4Address receiveAddress;
    /**
     * This method is used to print the content of a HELLOACK message.
     * \param os output stream
     */
    void Print (std::ostream &os) const;
    /**
...
private:
  ...
  struct
  {
    ...
    HelloAck helloAck;    //add 用来给hello回复确认包
...
public:
  ...
  HelloAck& GetHelloAck(){
    if(m_messageType == 0){
      m_messageType = HELLOACK_MESSAGE;
    }else{
      NS_ASSERT (m_messageType == HELLOACK_MESSAGE);
    }
    return m_message.helloAck;
  }
  ...
  /**
   * Get the HELLOACK message.
   * \returns The HELLOACK message.
   */
  const HelloAck& GetHelloAck () const
  {
    NS_ASSERT (m_messageType == HELLOACK_MESSAGE);
    return m_message.helloAck;
  }
