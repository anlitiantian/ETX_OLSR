...
RoutingExperiment::CommandSetup(int argc, char **argv)
{
    ...
    cmd.AddValue("minVelocity","节点运动的最小速度", minV);
    cmd.Parse(argc, argv);
...
void RoutingExperiment::Run(int nSinks, double txp, std::string CSVfileName)
{
    ...
    string s1 = "ns3::UniformRandomVariable[Min=";
    string s2 = "|Max=";
    string s3 = "]";
    // uint32_t min = 20;
    ostringstream oss;
    
    oss << s1 << minV << s2 << minV + 10 << s3;
    string meanV = oss.str();
    ...
    mobility.SetMobilityModel("ns3::GaussMarkovMobilityModel",
                              ...
                              "MeanVelocity", StringValue(meanV),
    ...
    string filename = "resultOfDiffVel.txt";
    ...
    out << (ReceivedPackets * 100) / SentPackets << " " 
        << AvgThroughput << " "
        << Delay.GetMilliSeconds() << " "
        << Jitter.GetMilliSeconds()
        << endl;
    out.close();
